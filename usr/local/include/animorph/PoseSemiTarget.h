...
private:
  ...
  Gen3D::Matrix::RotateAxis  axis;
  UsedVertex  modVertex;

  bool hasCenter;
  Gen3D::Vector3f center;
...
public:
  ...
  const Gen3D::Matrix::RotateAxis& getAxis () const {return axis;}

  bool getHasCenter() const {return hasCenter;}
  void setHasCenter(bool c) {hasCenter = c;}

  const Gen3D::Vector3f &getCenter() const {return center;}
  void setCenter(const Gen3D::Vector3f& c) {center = c;}
};
