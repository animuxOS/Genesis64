...
{
	# Title
	echo "Title: XviD MPEG-4 AVI Dual Pass (FFMPEG)"
...
execute()
{
	...
	case "$profile" in 
		...
		"1" ) 	ffmpeg -f dv -i pipe: -pass $pass -passlogfile "$file" $hq \
			-vcodec $xvid -g 300 $hq $progressive -s $full_res -aspect $aspect -b 2048$kilo \
			-acodec "$acodec" -ab 192$audio_kilo -y "$file".avi ;;
		"2" ) 	ffmpeg -f dv -i pipe: -pass $pass -passlogfile "$file" $hq \
			-vcodec $xvid -g 300 $hq $progressive -s $med_res -aspect $aspect -b 1024$kilo \
			-acodec "$acodec" -ar 44100 -ab 128$audio_kilo -y "$file".avi ;;
		"3" ) 	ffmpeg -f dv -i pipe: -pass $pass -passlogfile "$file" $hq \
			-vcodec $xvid -g 300 $hq $progressive -s $med_res -aspect $aspect -b 500$kilo \
			-acodec "$acodec" -ar 32000 -ab 64$audio_kilo -y "$file".avi ;;
		"4" ) 	ffmpeg -f dv -i pipe: -pass $pass -passlogfile "$file" $hq \
			-vcodec $xvid -r 12 -g 120 $hq $progressive -s $low_res -aspect $aspect -b 96$kilo \
