...
##  pyvnc2swf - edit.py
##
##  $Id: edit.py,v 1.1 2007/03/31 04:02:15 euske Exp $
##
...
def main(argv):
  ...
  for (k, v) in opts:
    if k == '-d':
      ...
      v = v.lower()
      if v not in ('shape','video','mpeg','mpg','flv','png','bmp','gif'):
  ...
  if streamtype == 'shape':
    ...
    stream = MPEGVideoStream(info)
  elif streamtype == 'flv':
    stream = FLVVideoStream(info)
  else:
    stream = ImageSequenceStream(info)
  try:
