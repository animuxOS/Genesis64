...
#Distro Title
	...
	def Global.autopartition=(value)
		@@autopartition = value
	end
	def Global.autopartition
		@@autopartition
	end
private
	@@source = "/LIVE"
...
def can_install
	if ((Global.boxname != "") and (Global.rootpass != nil) and (Global.user != "") and (Global.userpass != nil) and (not Global.chosenpart.empty? or Global.autopartition) ) 
		Global.can_install = true
	else
		Global.can_install = false
	end
puts Global.chosenpart.inspect
...
def setup_directories
	if Global.autopartition
	    system("swapoff -a")
	    disk = `/sbin/fdisk -l | grep -m 1 "^Disk /dev/" | awk '{print $2}' | sed 's/:.*//'`.chomp
	    disksize = `/sbin/fdisk -l #{disk} | grep "^Disk" | awk '{print $5}'`.chomp.to_i/1024
	    unitsize = `/sbin/fdisk -l #{disk} | grep "^Units" | awk '{print $9}'`.chomp.to_i/1024
	    rsize = (ROOT_SZ / unitsize + 1)*unitsize
	    ssize = (SWAP_SZ / unitsize + 1)*unitsize
	    homesize = disksize - rsize - ssize
	    rsize /= 1024
	    homesize /= 1024
    	    `sfdisk #{disk} -uM <<EOF
,#{rsize},83,*
...
EOF`
	    ...
	    Global.chosenpart << ["/","#{disk}1","ext3","yes","Linux"]
	    Global.chosenpart << ["/home","#{disk}2","xfs","yes","xfs"]
	    Global.chosenpart << ["swap","#{disk}3","swap","yes","swap"]
	end
...
puts "grep -q \"#{Global.source}\" /proc/mounts || mount -o loop /live/image/casper64/filesystem.squashfs \"#{Global.source}\""
system("grep -q \"#{Global.source}\" /proc/mounts || mount -o loop /live/image/casper64/filesystem.squashfs \"#{Global.source}\"")
...
=begin
autopartframe = Gtk::Frame.new("Partitions")
rightbox.pack_start(autopartframe, false, true, 0)
autopartbox = Gtk::VBox.new
autopartframe.add(autopartbox)
label = Gtk::Label.new.set_markup("<b>Auto partitioning mode</b>")
autopartbox.pack_start(label,false,false,0)
manualbt = StockButton.new("Click here to switch into manual mode", Gtk::Stock::HARDDISK, :horiz)
autopartbox.pack_start(manualbt,false,false,0)
=end
...
autopartbox = Gtk::VBox.new
partbox.add(autopartbox)
label = Gtk::Label.new.set_markup("<b>Auto partitioning mode</b>")
autopartbox.pack_start(label,false,false,10)
label = Gtk::Label.new.set_markup("All data on your first disk will bw lost!!!")
autopartbox.pack_start(label,false,false,10)
manualbt = StockButton.new("Click here to switch into manual mode", Gtk::Stock::PREFERENCES, :horiz)
autopartbox.pack_start(manualbt,true,false,0)

manualbt.signal_connect("clicked") do
#    autopartframe.hide
#    partframe.show    
    autopartbox.hide
    Global.autopartition = false
    partselbox.show
    partbuttonsbox.show
    partinfobox.show
    can_install
